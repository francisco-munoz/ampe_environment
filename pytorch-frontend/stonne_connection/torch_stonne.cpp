...
torch::Tensor simulated_sparsedense_matmul(std::string layer_name, torch::Tensor sparsemat1,  torch::Tensor mat2, int T_N, int T_K, std::string path_to_arch_file) {
    //Here starts the function
    //Creating config file to find out if we are going to run a dense or sparse simulation
    Config stonne_cfg; 
    if(path_to_arch_file != "") {
    ...
    std::cout << "MK dimensions: " << sparsemat1.sizes();
    std::cout << "Indices shape: " << sparsemat1._indices().sizes() << std::endl;
    int M = sparsemat1.sizes()[0];
    int K = sparsemat1.sizes()[1];
    int N = mat2.sizes()[1];
    std::cout << "M is " << M << std::endl;
    std::cout << "K is " << K << std::endl;
    std::cout << "N is " << N << std::endl;
    float* MK_sparse_raw = (float*) sparsemat1._values().data_ptr();
    float* KN_dense_raw = (float*) mat2.data_ptr();
    torch::Tensor output = torch::rand({M, N});
    float* output_raw = (float*) output.data_ptr();
    unsigned int nnz = sparsemat1._nnz();
    std::cout << "NNZ=" << nnz << std::endl;
    unsigned int*  sparse_col_id = new unsigned int [nnz+M];
    unsigned int*  MK_row_pointer = new unsigned int [M+1];
    unsigned int*  coo_col_id = new unsigned int [nnz+M]; //To fill one possible 0 per row
    unsigned int*  coo_row_id = new unsigned int [nnz+M]; //idem
    float*         MK_sparse_raw_with_zeros= new float[nnz+M];


    //Filling empty rows
    //Initializing first row
    long* col_pointer = (long*) sparsemat1._indices()[1][0].data_ptr();
    unsigned int col = (unsigned int)(*col_pointer);
    long* row_pointer = (long*) sparsemat1._indices()[0][0].data_ptr();
    unsigned int row = (unsigned int) (*row_pointer);
    float value = MK_sparse_raw[0];


    int index_new_vector=0;
    if(row==0) {
        coo_col_id[0]=col;
	coo_row_id[0]=row;
        MK_sparse_raw_with_zeros[0]=value;
	index_new_vector++;
    }
    else { //Insert as many 0's as rows up to the first row used
        for(int z=0; z<row; z++) {
	    std::cout << "Entering the first loop" << std::endl;
            coo_col_id[index_new_vector]=0;
	    coo_row_id[index_new_vector]=z;
	    MK_sparse_raw_with_zeros[index_new_vector]=0.0;
	    index_new_vector++;
	}
	//Adding the first value after this padding
	coo_col_id[index_new_vector]=col;
        coo_row_id[index_new_vector]=row;
        MK_sparse_raw_with_zeros[index_new_vector]=value;	
	index_new_vector++;
    }
    for(int i=1; i<nnz; i++) { 
       long* col_pointer = (long*) sparsemat1._indices()[1][i].data_ptr();
       unsigned int col = (unsigned int)(*col_pointer);
       long* row_pointer = (long*) sparsemat1._indices()[0][i].data_ptr();
       unsigned int row = (unsigned int) (*row_pointer); 
       long* previous_row_pointer = (long*) sparsemat1._indices()[0][i-1].data_ptr();
       unsigned int previous_row = (unsigned int) (*previous_row_pointer);
       float value = MK_sparse_raw[i];

       int diff = row - previous_row; 
       //Looking if zeros are needed
       for(int z=0; z<(diff-1); z++) { //note that this loop only works if diff > 1. Diff=0 same row. diff=1 consecutive rows. diff > 1 there are empty rows
	   std::cout << "A zero is inserted" << std::endl;
           coo_col_id[index_new_vector]=0;
           coo_row_id[index_new_vector]=previous_row+1+z;
           MK_sparse_raw_with_zeros[index_new_vector]=0.0;
           index_new_vector++;
       }
       //Once finished, we insert the i row
       coo_col_id[index_new_vector]=col;
       coo_row_id[index_new_vector]=row;
       MK_sparse_raw_with_zeros[index_new_vector]=value;
       index_new_vector++;
    ...
    //If there are remaining rows in the end we do the same
    long* last_row_pointer = (long*) sparsemat1._indices()[0][nnz-1].data_ptr();
    unsigned int last_row = (unsigned int) (*last_row_pointer);
    for(int i=last_row+1; i<M; i++) {
        coo_col_id[index_new_vector]=0;
	coo_row_id[index_new_vector]=i;
	MK_sparse_raw_with_zeros[index_new_vector]=0.0;
	index_new_vector++;
	std::cout << "This last loop is executed" << std::endl;
    }

    int new_nnz = index_new_vector;
    std::cout << "NEW NNZ IS " << new_nnz << std::endl;

    //End of the procedure to redimension the nnz vectors and insert the empty rows

    //Starting the algorithm to transform COO with no emtpy rows to CSR
    //Initializing MK_row_pointer to ensure the correctness of this algorithm
    for(int i=0; i<(M+1); i++) {
        MK_row_pointer[i]=0;
    }
    for(int i=0; i<new_nnz; i++) {
	unsigned int col = coo_col_id[i];
	sparse_col_id[i]=col;
	unsigned int row = coo_row_id[i];
	//if(i<100) {
        //    std::cout << "ROW_STONNE= " << row << "  COL_STONNE= " << col << std::endl;
	//}
	if((row>0) && (MK_row_pointer[row] == 0)) {
	    //std::cout << "Updating row " << row << std::endl;
            MK_row_pointer[row]=i; //First row apparition
	}
	//if(i<100) {
         //   std::cout << "MK ROW POINTER " << row << " " << MK_row_pointer[row] << std::endl;
//	}
    }
    MK_row_pointer[M]=new_nnz; // Allow M+1 - M calculation with no out of range
    std::cout << "Llega al punto 1" << std::endl;



    //Running stonne 
    simulateSparseDenseGemm(layer_name, MK_sparse_raw_with_zeros, KN_dense_raw, output_raw, M, K, N, sparse_col_id, MK_row_pointer, T_N, T_K, stonne_cfg);
    std::cout << "Llega al punto 2" << std::endl;

    std::cout << "Executed with exit" << std::endl;
    /*std::cout << "Sparse Col ID: " << std::endl;
    for(int i=0; i<nnz; i++) {
        std::cout << sparse_col_id[i] << " ";
    }
    std::cout << std::endl;

    std::cout <<  "MK Row pointer: " << std::endl;
    for(int i=0; i<(M+1); i++) {
    ...
    */
    ...
    if(stonne_cfg.sparsitySupportEnabled()) {
      simulateSparseGemmWithBitmapsForward(layer_name, KN_weight_raw, MK_input_raw, output_raw, 1, 1, gemm_M, gemm_K, gemm_N, MK_bitmap, KN_bitmap,  stonne_cfg, MK_STA_KN_STR);
    }
	...
	//KN_weight_raw = (float*) weight.data_ptr();
        ...
        //Raise exception
	std::err << "The architecture is not configured to support a sparse execution. Please, use a sparse controller instead."
    }

    return output;
*/
   delete[] sparse_col_id;
   delete[] MK_row_pointer;
   delete[] coo_col_id;
   delete[] coo_row_id;
   delete[] MK_sparse_raw_with_zeros;
   return output; 
}
